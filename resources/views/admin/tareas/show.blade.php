		...
		<table class="table table-hover ml-4 mt-4">
			...
			<tbody>
				...
				<tr>
					...
					<td><a href="{{asset("entregas/$entregas->file")}}">{{$entregas->file}}</a></td>
					<form action="{{route('entregas.update',$entregas->id)}}" method="POST">
						@method('PATCH')
						@csrf
						<td>
							<textarea name="comments" id="">{{$entregas->comments}}</textarea>
						</td>
						<td>
							<input type="number" min="0" max="10" name="cal" value="{{$entregas->cal}}">
						</td>
						<td>
